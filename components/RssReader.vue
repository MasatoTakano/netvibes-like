...
<template>
  ...
  >
    ...
    <div v-else-if="error" class="error-message">
      ...
      <!-- エラー詳細を表示 -->
      <p class="error-details">{{ error.data?.message || error.message || 'Unknown error' }}</p>
      <p>URL: {{ props.feedUrl }}</p>
    </div>
    <div v-else-if="!feedData || !feedData.items || feedData.items.length === 0" class="empty-message">
      {{ $t('rssReader.noItems') }}
    </div>
    <ul v-else class="feed-items">
      <!-- itemCount に基づいて表示件数を制限 -->
      <li v-for="(item, index) in displayedItems" :key="item.guid || item.link || index" class="feed-item">
        <span v-if="item.pubDate" class="item-time">{{ formatTime(item.pubDate) }}</span>
        <a :href="item.link" target="_blank" rel="noopener noreferrer" class="item-title" :title="item.title || ''">
...
const props = defineProps({
  ...
  updateIntervalMinutes: { type: Number as () => number | null, default: DEFAULT_RSS_UPDATE_INTERVAL_MINUTES },
...
const rssStyle = computed(() => {
  ...
  if (props.fontFamily) { // 値があれば設定
    style.fontFamily = props.fontFamily;
  }
  if (props.fontSize) { // 値があれば設定
...
const fetchFeed = async (url: string) => {
  ...
  try {
    ...
    const { data: fetchedData, error: fetchError, pending: fetchPending } = await useFetch<FeedData>(apiUrl, {
    ...
    console.error('[ERROR] [RssReader] Unexpected error during fetchFeed execution:', catchedError);
...
watch([() => props.feedUrl, () => props.updateIntervalMinutes], ([newUrl], [oldUrl]) => {
