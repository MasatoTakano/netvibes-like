...
<template>
  <div class="calendar-widget-content">
    ...
    <div v-else v-html="sanitizedIframeTag" class="iframe-container" :class="{ 'dark-mode-iframe': isDarkMode }"></div>
  </div>
</template>
...
const sanitizedIframeTag = computed(() => {
  ...
  return DOMPurify.sanitize(props.iframeTag, {
    ...
    ADD_ATTR: ['src', 'style', 'frameborder', 'scrolling', 'allowfullscreen', 'loading', 'referrerpolicy', 'width', 'height'], // 許可する属性を追加
  });
});

watch(() => props.iframeTag, (newTag) => {
