...
<template>
  <BaseModal :show="show" title-id="rss-settings-title" @close="close">
    <template #title>{{ $t('rssSettings.title') }}</template>

    <template #default>
      <div v-if="widgetData" class="settings-form">
        <div class="form-group">
          <label :for="'rss-url-' + widgetData.id">{{
            $t('rssSettings.feedUrl')
          }}</label>
          <input
        ...
        <div class="form-group">
          <label :for="'rss-count-' + widgetData.id">{{
            $t('rssSettings.itemCount')
          }}</label>
          <input
        ...
        <div class="form-group">
          <label :for="'rss-font-family-' + widgetData.id">{{
            $t('common.font')
          }}</label>
          <select
        ...
        <div class="form-group">
          <label :for="'rss-font-size-' + widgetData.id">{{
            $t('common.fontSize')
          }}</label>
          <input
            :id="'rss-font-size-' + widgetData.id"
        ...
        <div class="form-group">
          <label :for="'rss-interval-' + widgetData.id">{{
            $t('rssSettings.updateInterval')
          }}</label>
          <select
            :id="'rss-interval-' + widgetData.id"
            v-model.number="editableSettings.updateIntervalMinutes"
          >
            ...
            <option :value="null">{{ $t('rssSettings.manualOnly') }}</option>
            <!-- 更新しないオプション -->
          </select>
        </div>
...
<script setup lang="ts">
  ...
  watch(
    ...
    (newData) => {
      if (newData) {
        ...
        ((editableSettings.itemCount =
          newData.itemCount || DEFAULT_RSS_ITEM_COUNT),
          (editableSettings.fontFamily =
            newData.fontFamily || DEFAULT_RSS_FONT_FAMILY));
        editableSettings.fontSize = newData.fontSize || DEFAULT_RSS_FONT_SIZE;
        editableSettings.updateIntervalMinutes =
          newData.updateIntervalMinutes || DEFAULT_RSS_UPDATE_INTERVAL_MINUTES;
  ...
  const validateSettings = (): boolean => {
    error.value = null;
    if (
      !editableSettings.feedUrl ||
      (!editableSettings.feedUrl.startsWith('http://') &&
        !editableSettings.feedUrl.startsWith('https://'))
    ) {
      error.value = 'Please enter a valid URL.';
      return false;
    }
    if (
      typeof editableSettings.itemCount !== 'number' ||
      editableSettings.itemCount < 1
    ) {
      error.value = t('rssSettings.invalidItemCount');
      return false;
    }
    ...
    if (interval !== null && (typeof interval !== 'number' || interval < 5)) {
      error.value = 'Invalid update interval selected.';
      return false;
    }
