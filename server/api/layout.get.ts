...
export default defineEventHandler(async (event) => {
  ...
  try {
    console.log(
      `[API GET /api/layout] Attempting to load layout for user ${userId}...`,
    );
    ...
    if (layoutRecord && layoutRecord.data) {
      // データが存在する場合
      console.log(
        `[API GET /api/layout] Found layout data (updated_at: ${layoutRecord.updatedAt}) for user ${userId}.`,
      );
      ...
      try {
        ...
        // JSONパースエラー
        console.error(
          `[API GET /api/layout] Error parsing layout data for user ${userId}:`,
          parseError,
        );
        console.error(
          '[API GET /api/layout] Faulty JSON string:',
          layoutRecord.data,
        );
      ...
      if (!layoutRecord) {
        console.log(
          `[API GET /api/layout] No layout record found in DB for user ${userId}. Returning default layout.`,
        );
      } else {
        // layoutRecord は存在するが data が空の場合
        console.log(
          `[API GET /api/layout] Layout record found, but data is empty for user ${userId}. Returning default layout.`,
        );
      }
      // デフォルトのレイアウトデータを返す
    ...
    console.error(
      `[API GET /api/layout] Database error fetching layout for user ${userId}:`,
      dbError,
    );
    throw createError({
      statusCode: 500,
