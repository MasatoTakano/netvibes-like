...
import { defineEventHandler, createError, getCookie, setCookie, readBody } from 'h3'; // readBody もインポート
...
export default defineEventHandler(async (event) => {
  ...
  console.log(`[API POST /api/layout] Extracted userId:`, userId, `(Type: ${typeof userId})`);

  // userId が undefined でないことを確認してから DB 操作へ進むチェックを追加 (より安全)
  if (typeof userId !== 'string' || userId.length === 0) {
    console.error(`[API POST /api/layout] userId is invalid or undefined after validation! userId: ${userId}`);
    throw createError({ statusCode: 500, statusMessage: 'Internal Server Error: User ID is missing in session'});
  }

  console.log(`[API POST /api/layout] Authenticated user confirmed with valid userId: ${userId}`);
  ...
  try {
    ...
    if (!Array.isArray(layoutDataFromClient)) {
      ...
      console.error(`[API POST /api/layout] Error reading request body for user ${userId}:`, readError);
      throw createError({ statusCode: 400, statusMessage: 'Bad Request: Could not read request body', message: readError.message });
  }
  ...
  try {
    ...
    console.log(`[API POST /api/layout] Stringified layout data length for user ${userId}: ${layoutDataString.length}`);
    ...
    const upsertResult = await prisma.layout.upsert({
      ...
      create: { // 存在しない場合の作成データ
        userId: userId,
        data: layoutDataString,
      },
      select: { // 結果として何を取得するか (デバッグ用に updatedAt を取得)
          updatedAt: true,
      }
    });
